...
describe("API - Dish CRUD Operations", () => {
  ...
  describe("GET Operations", () => {
    it("Успешное получение блюда по ID", async () => {
      const createRes = await request(app).post("/api/dishes").send({
        name: "!второе Плов",
        photos: [],
        size: 200,
    ...
    it("Ошибка при получении несуществующего блюда (граница: non-existent ID)", async () => {
      const res = await request(app).get("/api/dishes/99999");
  ...
  describe("PUT Operations", () => {
    it("Успешное обновление имени блюда", async () => {
      ...
      const dishId = createRes.body.dish.id;
      const res = await request(app).put(`/api/dishes/${dishId}`).send({
        name: "Рис на гарнир",
        size: 150,
        items: [{ productId: product1.id, quantity: 100 }],
      });
    ...
    it("Ошибка при обновлении несуществующего блюда", async () => {
      const res = await request(app).put("/api/dishes/99999").send({
        name: "Новое имя",
        size: 150,
        items: [{ productId: product1.id, quantity: 100 }],
      });
  ...
  describe("DELETE Operations", () => {
    it("Успешное удаление блюда", async () => {
      ...
      const dishId = createRes.body.dish.id;
      const deleteRes = await request(app).delete(`/api/dishes/${dishId}`);

      expect(deleteRes.status).toBe(204);

      const getRes = await request(app).get(`/api/dishes/${dishId}`);
      expect(getRes.status).toBe(404);
    });

    it("Ошибка при удалении несуществующего блюда", async () => {
      const res = await request(app).delete("/api/dishes/99999");
